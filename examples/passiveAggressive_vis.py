...
reg = PassiveAggressiveRegressor(C=.01, max_iter=1000, tol=1e-4)
...
for i, (coef, intercept) in enumerate(reg.steps_):
    ...
    plt.plot(X, y_pred, label=f"Iteration {i}", color=colors[i] if i < len(colors) else "red")
...
plt.tight_layout()
plt.savefig('plots/passiveAggressive.png', dpi=300)