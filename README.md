...
&emsp; [Polynomial Transformations](#polynomial-transformation)
...
It is called "passive-aggressive" because:
- Passive: If the prediction is correct, the model remains unchanged.
- Aggressive: If the prediction is incorrect, the model is updated to correct the mistake.
...
```
### Passive Aggressive Regressor Visualized
The plot below shows the progression of the Passive Aggressive Regressor through each iteration step. With the last step shown in red.

![alt text](https://raw.githubusercontent.com/SantiagoEnriqueGA/custom_linear_learning/refs/heads/main/plots/passiveAggressive.png)
...
Polynomial Transformation is a technique used to transform the original features into polynomial features of a specified degree. This transformation allows linear models to fit more complex, non-linear relationships by adding polynomial terms to the feature set. For example, a quadratic transformation will add squared terms of the original features, and a cubic transformation will add cubic terms.

Polynomial transformations are useful in scenarios where the relationship between the independent and dependent variables is non-linear. By transforming the features, linear regression models can capture these non-linear relationships and improve their predictive performance.

### Usage Example
```python
from linearModels import PolynomialTransform, OrdinaryLeastSquares
import numpy as np
from sklearn.metrics import r2_score
...
```
Example Usage Polynomial Transform of Features
Base Model: y = 10 * sin(x) + noise
OLS R^2 Score: 0.015
OLS Polynomial Degree 2 R^2 Score: 0.87
OLS Polynomial Degree 3 R^2 Score: 0.98
```

### Polynomial Transformation Visualized
![alt text](https://raw.githubusercontent.com/SantiagoEnriqueGA/custom_linear_learning/refs/heads/main/plots/polynomialTransform.png)